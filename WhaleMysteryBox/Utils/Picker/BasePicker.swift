...
protocol PickerProtocol{
    ...
    var selectedHandler: (SelectedType, UIView) -> () { get set }
...
class BasePicker<DataType: Equatable, SelectedType> : BaseView, PickerProtocol{
    ...
    var selectedHandler: (SelectedType,UIView) -> ()
    ...
    init(title:NSAttributedString,selectedHandler:@escaping (SelectedType,UIView) -> ()) {
        self.title = title
    ...
    lazy var header: UIView = {
        ...
        comfirmBtn.addBlock(for: .touchUpInside) { [weak self] _ in
            if let self = self{
                self.selectedHandler(self.selectedData!,self)
            }
        }
